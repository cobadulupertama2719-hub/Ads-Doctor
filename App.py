...
# Konfigurasi Demo
DEMO_DURATION_MINUTES = 10  # 10 menit demo
...
def show_login_or_demo():
    ...
    with col2:
        st.markdown("### 🎁 **Coba Demo 10 Menit**")
        st.markdown(f"""
        <div style="background: #f0f0ff; padding: 1rem; border-radius: 0.8rem;">
            <small>✅ Gratis 10 menit akses penuh<br>
            ✅ Maks {MAX_DEMO_ANALYSIS} kali analisis<br>
            ✅ Semua fitur premium bisa dicoba<br>
    ...
    if st.session_state.get("demo_mode", False):
        ...
        if remaining <= 120:
            ...
            <div style="background: #fee2e2; padding: 0.5rem; border-radius: 0.5rem; text-align: center; margin-bottom: 1rem;">
                ...
                <a href="#" style="color: #dc2626;">Beli sekarang →</a></small>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.sidebar.markdown(f"""
            <div style="background: #f0f0ff; padding: 0.5rem; border-radius: 0.5rem; text-align: center; margin-bottom: 1rem;">
                ...
                Sisa: {minutes:02d}:{seconds:02d}<br>
                Analisis: {st.session_state.get("demo_analysis_count", 0)}/{MAX_DEMO_ANALYSIS}</small>
            </div>
            """, unsafe_allow_html=True)
...
# Jika demo sudah mencapai batas analisis
...
# ==================== TAMPILAN UTAMA ====================
# Tombol Logout (hanya untuk premium)
...
with col_logout3:
    if is_premium_user():
        ...
        st.markdown('<span class="premium-badge">🎁 DEMO MODE</span>', unsafe_allow_html=True)
...
# Tombol Analisis dengan hitungan demo
analyze_clicked = st.button("🔍 Analisis Iklan", use_container_width=True)

if analyze_clicked:
    # Increment demo analysis count
    if st.session_state.get("demo_mode", False):
    ...
    # Jika demo, tampilkan pesan beli
    if st.session_state.get("demo_mode", False):
        st.markdown("---")
        st.markdown("""
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 1rem; border-radius: 0.8rem; text-align: center; color: white;">
            <h4 style="margin: 0;">💎 Suka dengan tools ini?</h4>
            <p style="margin: 0.5rem 0;">Dapatkan akses penuh + konsultasi GRATIS!</p>
            <p style="font-size: 1.5rem; font-weight: bold; margin: 0.5rem 0;">Rp147rb (sekali bayar)</p>
            <a href="https://wa.me/6281234567890?text=Halo%20Arkidigital%2C%20saya%20mau%20beli%20Ads%20Doctor%20Premium%20Rp147rb" target="_blank">
                <button style="background: white; color: #667eea; border: none; padding: 0.5rem 1.5rem; border-radius: 2rem; font-weight: bold; cursor: pointer;">🔥 Beli Sekarang</button>
            </a>
        </div>
        """, unsafe_allow_html=True)
...
<div class="footer">
    ...
    {'<p>🎁 Mode Demo - Beli premium untuk akses penuh!</p>' if st.session_state.get("demo_mode", False) else ''}
</div>
""", unsafe_allow_html=True)
